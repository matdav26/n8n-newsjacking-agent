...
```mermaid
flowchart TD
    A[RSS Feeds] --> B[Time Filter]
    B --> C[Keyword Filter]
    C --> D[Article Formatter]
    D --> E[Channel Classifier (LLM)]
    E --> F[Channel Router]

    F --> G1[Twitter Bucket]
    F --> G2[LinkedIn Bucket]
    F --> G3[Dual Bucket]

    G1 --> H1[Twitter Ranking Engine]
    G2 --> H2[LinkedIn Ranking Engine]
    G3 --> H3[Dual Ranking Engine]

    H1 --> I[Ranked Results Merger]
    H2 --> I
    H3 --> I

    I --> J[Curator Engine]

    J --> K[LinkedIn Post Generator]
    J --> L[Twitter Post Generator]

    K --> M[Unified Post Merger]
    L --> M

    M --> N[Digest Email Generator]
